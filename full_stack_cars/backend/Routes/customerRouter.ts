...
customerRouter.get("/OCat",async (request:Request,response:Response,next:NextFunction)=>{
    try{
        const catsWithO = await customerLogic.getCategoriesWithO();
        response.status(200).json(catsWithO);
    } catch (err){
        next (err);
    }
})
